...
// jx: 可吃 integer or date string
Job.prototype.expires = function( exp ) {
  ...
  setImmediate(function() {
	...
	self.save(function() {
	  var jobCallback = function(err) {
		...
		// begin jx code
		// console.log( 'this: ', self.attrs )
		// debugger // 看 logs[] 有無更新

		if( self.attrs.expires ){

			var exp = self.attrs.expires
			var logs = self.attrs.logs
			var shouldRemove = false

			logs.push( self.attrs.lastRunAt ) // 記錄本次執行日期

			if( typeof exp == 'string' ){
				// 日期
				if( new Date(exp) >= Date.now() ) shouldRemove = true
			}else{
				// 次數
				if( logs.length >= exp ) shouldRemove = true
			}

			if( shouldRemove ){
				return self.remove( function(err){
					if(err) console.error( 'expire job failed' )
					console.log( 'expire job removed!' )
					// debugger
				})
			}
		}// end jx code
